...
func NewRouter(taskContext *delegate.TaskContext) *task.Router {
	...
	r.RegisterFunc("secret/vault/create", vault.CreateHandler)
	r.RegisterFunc("secret/vault/rename", vault.RenameHandler)
	r.RegisterFunc("secret/vault/update", vault.UpdateHandler)
	r.RegisterFunc("secret/vault/delete", vault.DeleteHandler)
	r.RegisterFunc("secret/vault/validate", vault.ValidateHandler)
