...
func initSystem(ctx context.Context, config *delegate.Config) (*server.System, error) {
	...
	metrics := metricsinjection.ProvideMetricsClient(config)
	daemonSetReconciler := daemonset.ProvideDaemonSetReconciler(daemonSetManager, taskRouter, clientClient, metrics)
	pollerPoller := poller.ProvidePoller(clientClient, taskRouter, config, metrics)
	keepAlive := heartbeat.ProvideKeepAlive(config, clientClient, metrics)
